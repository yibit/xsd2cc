...
TypeBase::TypeBase(const Xsd& xsd, const XmlNode& node,
                   const std::string& ns_prefix, const std::string& type)
    : xsd_(xsd),
      node_(node),
      ns_href_(ns_prefix),
      type_(type),
...
const std::string TypeBase::CType() const { return c_type_; }
