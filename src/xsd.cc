...
Xsd::Xsd(Application& app, XmlDocument& doc) 
    : app_(app), doc_(doc)
{
  ParseElements(doc);
}
...
XmlNode Xsd::GetPortTypeOp(const std::string& portname, const std::string& op) const
{
  XmlNode n(doc_, "portType");
  while (n) {
    if (n.GetProperty("name") == portname || portname.empty()) {
...
XmlNode Xsd::GetBindingOp(const std::string& portname, const std::string& op) const
{
...
void Xsd::ShowElement(const XmlNode& element_node, const std::string& level)
{
  ...
  if (!type.empty()) {
    ...
    tmp += type;
    while (tmp.size() < 24 + level.size())
      tmp += " ";
    tmp += " \"" + name + "\"";
    while (tmp.size() < 40 + level.size())
      tmp += " ";
...
XmlNode Xsd::FindComplexType(const std::string& ns_href, const std::string& type) const
{
  ...
  std::map<std::string, XmlNode>::const_iterator it2 = ref.find(Split(type).Type);
  if (it2 != ref.end()) {
    return it2 -> second;
  }
...
XmlNode Xsd::FindSimpleType(const std::string& ns_href, const std::string& type) const
{
  std::map<std::string, std::map<std::string, XmlNode> >::const_iterator it;
...
std::list<std::pair<std::string, XmlNode> > 
Xsd::GetMessageParameters(const std::string& msgname, 
...
void Xsd::ParseElements(XmlDocument& doc, const std::string& targetNs)
{
  ...
  if (root.GetNodeName() == "schema") {
    ...
    while (n) {
      std::string ns = n.PropertyExists("namespace") ? n.GetProperty("namespace") : "";
      std::string loc = n.PropertyExists("schemaLocation") ? n.GetProperty("schemaLocation") : "";
  ...
  if (n) {
    ...
    while (n2) {
      ...
      while (n3) {
        std::string ns = n3.PropertyExists("namespace") ? n3.GetProperty("namespace") : "";
        std::string loc = n3.PropertyExists("schemaLocation") ? n3.GetProperty("schemaLocation") : "";
        std::cout << "Import " << ns << " schemaLocation " << loc << std::endl;
        Utility::Path current = Utility::CurrentDirectory();
        Utility::Path path(loc);
        if (!path.GetPath().empty()) {
  ...
  while (n) {
    ...
    if (n.PropertyExists("type")) {
      ...
      if (complex) {
        ...
        complexType_Map_[ns_href][type] = ParseComplexType(complex, ns_href, type);
...
XmlNode Xsd::ParseComplexType(const XmlNode& node, 
...
const std::list<std::string>& Xsd::ComplexTypes(const std::string& ns_href) const
{
  std::map<std::string, std::list<std::string> >::const_iterator it;
  it = complexType_.find(ns_href);
  if (it != complexType_.end())
    return it -> second;
...
const std::list<std::string>& Xsd::SimpleTypes(const std::string& ns_href) const
{
  std::map<std::string, std::list<std::string> >::const_iterator it;
  it = simpleType_.find(ns_href);
  if (it != simpleType_.end())
    return it -> second;
