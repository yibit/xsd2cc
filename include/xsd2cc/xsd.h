...
class NSFile {
 public:
  NSFile(FILE* file, const std::string& ns, const std::string& href = "")
      : file_(file), ns_(ns), href_(href), start_(false) {}
...
class Xsd {
 public:
  ...
  std::list<std::pair<std::string, XmlNode> > GetMessageParameters(
      const std::string& msgname, std::string& path) const;

  XmlNode GetPortTypeOp(const std::string& portname,
                        const std::string& op) const;
  XmlNode GetBindingOp(const std::string& portname,
                       const std::string& op) const;
  ...
  XmlNode FindComplexType(const std::string& ns_href,
                          const std::string& type) const;
  XmlNode FindSimpleType(const std::string& ns_href,
                         const std::string& type) const;
  ...
  const std::string GetParent(const std::string& href,
 ...
 private:
  ...
  void ParseElements(const XmlNode& node, const std::string& ns_href,
                     const std::string& parent_class = "");
  void ParseTypes(const XmlNode& node, const std::string& ns_href);
  XmlNode ParseComplexType(const XmlNode& node, const std::string& ns_href,
